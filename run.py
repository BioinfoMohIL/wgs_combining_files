...
def highlighting_cells(ws, cells, bad_quality_samples=None):
    yellow_fill = PatternFill(start_color='FFFF00', end_color='FFFF00', fill_type='solid')  # yellow
    red_fill = PatternFill(start_color='FF9999', end_color='FF9999', fill_type='solid')     # pastel red

    for row in ws.iter_rows():
        for cell in row:
            if bad_quality_samples and cell.value in bad_quality_samples:
                cell.fill = red_fill
            elif cell.value in cells:
...
def create_tables(data_folder, df_sorted, output_file, bad_quality_samples=None):
    ...
    with pd.ExcelWriter(output_file,  engine='openpyxl', mode='a') as writer:
        ...
        for file_name in files:
            ...
            # Yellow indicate the location of the priority samples, red didnt pass the threshold
            highlighting_cells(ws, cells, bad_quality_samples)
...
def load_elab_data(data_dir, df_filtered):
    elab_dfs = []

    for filename in os.listdir(data_dir):
        if 'elab' in filename.lower() and filename.endswith(('.xlsx', '.xls')):
            ...
            try:
                ...
                for idx, row in raw_df.iterrows():
                    if row.astype(str).str.contains('Extraction Date', case=False).any():
                        header_row = idx
                        break

                if header_row is None:
                    print(f"Skipping {filename}: 'Extraction Date' not found.")
                    continue
                ...
                if 'Sample' in df.columns:
                    filtered_df = df[df['Sample'].astype(str).isin(df_filtered[SAMPLES_CODES_COL].astype(str))]
                    elab_dfs.append(filtered_df)
                else:
                    print(f"Skipping {filename}: 'Sample' column not found.")

            except Exception as e:
                print(f"Error processing {filename}: {e}")

    return pd.concat(elab_dfs, ignore_index=True) if elab_dfs else pd.DataFrame()
...
filtered_df = combined_df[
    ...
    ((combined_df['260/280'] < 1.7) | (combined_df['260/280'] > 2))
]
...
try:
    create_tables(data_dir, df_sorted, results_file, bad_quality_samples)
